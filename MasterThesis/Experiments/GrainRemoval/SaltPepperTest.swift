...
class SaltPepperTest: Experiment {
    ...
    func work() -> (CGPGraph, History) {
        ...
        let (best, history) = population.process(withDatasource: dataSource,
                                                 runParameters: .init(generations: 800, error: 0.005)) { graph, iteration in

            guard iteration % 150 == 0 else {
                return
            }
        ...
        let image = Image<UInt8>(width: dataSource.fullSize, height: dataSource.fullSize, pixels: bestPixels)
